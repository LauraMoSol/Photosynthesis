...
# NOTE HERE AND NOT WORKING
# --- Step 9: Draw overlay text for each slice individually and attach to correct slice ---
font_size = max(10, int(height * 0.03))
spacing = int(font_size * 1.5)
font = Font("SansSerif", Font.PLAIN, font_size)
x, y = 10, 10

# Create a single overlay object that holds ROIs assigned to specific slices
    ...
    for p in selected:
        if p == "Elapsed Time":
            ...
            if elapsed is None:
                ...
                value = "{:.2f} min".format(elapsed / 60.0)
        else:
        ...
        roi.setPosition(i + 1)  # Set slice index (1-based)
        combined_overlay.add(roi)
        ypos += spacing

# Apply combined overlay with per-slice ROIs
stack_imp.setOverlay(combined_overlay)
